...
RADIATION_LENGTH_FROM_Z = {
    ...
    "Mn": 14.64, "Fe": 13.84, "Co": 13.62, "Ni": 12.68, "Cu": 12.86, "Zn": 12.43,
    "Ga": 12.47, "Ge": 12.25, "As": 11.94, "Se": 11.91, "Br": 11.42, "Kr": 11.37,
    "Rb": 11.03, "Sr": 10.76, "Y":  10.41, "Zr": 10.20, "Nb": 9.92,  "Mo": 9.80,
    "Tc": 9.58,  "Ru": 9.48,  "Rh": 9.27,  "Pd": 9.20,  "Ag": 8.97,  "Cd": 9.00,
    "In": 8.85,  "Sn": 8.82,  "Sb": 8.73,  "Te": 8.83,  "I":  8.48,  "Xe": 8.48,
    "Cs": 8.31,  "Ba": 8.31,  "La": 8.14,  "Ce": 7.96,  "Pr": 7.76,  "Nd": 7.71,
    "Pm": 7.51,  "Sm": 7.57,  "Eu": 7.44,  "Gd": 7.48,  "Tb": 7.36,  "Dy": 7.32,
    "Ho": 7.23,  "Er": 7.14,  "Tm": 7.03,  "Yb": 7.02,  "Lu": 6.92,  "Hf": 6.89,  
    "Ta": 6.82,  "W":  6.76,  "Re": 6.69,  "Os": 6.68,  "Ir": 6.59,  "Pt": 6.54, 
    "Au": 6.46,  "Hg": 6.44,  "Tl": 6.42,  "Pb": 6.37,  "Bi": 6.29,  "Po": 6.16, 
...
# Isotopes to generate stopping-power-only HDF5 files for when no ACE file
# exists. Covers H and He which TENDL excludes because TALYS doesn't apply.
# Format: (symbol, A, atomic_weight_ratio)
# AWR = atomic mass / neutron mass; neutron mass = 1.008664916 u
PSTAR_ONLY_ISOTOPES = [
    ("H",  1, 1.00794  / 1.008664916),   # natural H  ≈ H-1
    ("H",  2, 2.01410  / 1.008664916),   # deuterium
    ("He", 3, 3.01603  / 1.008664916),   # He-3
    ("He", 4, 4.00260  / 1.008664916),   # He-4
]
...
# Temperature written to all files (TENDL proton ACE files report 0 K)
T_KELVIN = 0.0
...
def write_stopping_power(file, pstar_dir, symbol, verbose=False):
    """
    Write stopping_power group into an open HDF5 file if a PSTAR file exists.
    Returns True if data was written.
    """
    if pstar_dir is None:
        return False
    pstar_path = os.path.join(pstar_dir, f"{symbol}.txt")
    if not os.path.exists(pstar_path):
        if verbose:
...
def _write_kalbach_mann(km_data, h5_group):
    ...
    h5_group.create_dataset(
        "energy", data=np.array(km_data.incident_energies)
    ).attrs["unit"] = "MeV"
    ...
    h5_group.create_dataset(
        "energy_out", data=np.array(energy_out)
    ).attrs["unit"] = "MeV"
...
def load_secondary_particles(ace_table, file, verbose=False):
    ...
    pi_method = next(
        (c for c in ["particle_identifier", "ZAP", "type", "particle_type"]
    ...
    for i in range(1, n_types + 1):
        ...
        off_method = next(
            (c for c in ["energy_index", "offset", "locator", "index"] if hasattr(xs_i, c)),
            None
        )
        edy_method = next(
            (c for c in ["energy_distribution_data", "distribution_data", "distribution"]
             if hasattr(edy_i, c)),
            None
        )
        ...
        for j in range(1, n_rx + 1):
            ...
            rf     = ("LAB" if rf_raw == ACEtk.ReferenceFrame.Laboratory else
                      "COM" if rf_raw == ACEtk.ReferenceFrame.CentreOfMass else str(rf_raw))
            ...
            if xs_method and off_method:
                ...
                ds.attrs["unit"]   = "barns"
                if verbose:
                    print(f"        [warn] xs methods not found: "
                          f"{[x for x in dir(xs_i) if not x.startswith('_')]}")

            if edy_method:
                try:
                    _write_kalbach_mann(
                        getattr(edy_i, edy_method)(j),
                        mt.create_group("kalbach_mann")
                    )
                except Exception as exc:
                    if verbose:
                        print(f"        [warn] energy dist: {exc}")
            elif verbose:
                print(f"        [warn] edy method not found: "
...
def process_ace_file(ace_path, output_dir, pstar_dir=None, verbose=False):
    ...
    if symbol == "H2":
        radiation_length = 125.98
    else:
        radiation_length = RADIATION_LENGTH_FROM_Z.get(symbol)
    ...
    write_stopping_power(file, pstar_dir, symbol, verbose=verbose)
    ...
    for mts, grp in [(capture_MTs,    capture_group),
        ...
        for MT in mts:
            ...
            rf_str = ("LAB" if rf == ACEtk.ReferenceFrame.Laboratory else
    ...
    if not load_cosine_distribution(angle_block.angular_distribution_data(0), ag) \
            and verbose:
    ...
    for mts, grp in [(inelastic_MTs, inelastic_group),
        ...
        for MT in mts:
            ...
            if not load_cosine_distribution(
                    angle_block.angular_distribution_data(idx), ag) and verbose:
    ...
    for mts, grp in [(inelastic_MTs, inelastic_group),
        ...
        for MT in mts:
            ...
            if not isinstance(data, ACEtk.continuous.MultiDistributionData):
                ...
                elif (all(p.number_interpolation_regions == 1 for p in probs)
                      and all(p.interpolants[0] == 1 for p in probs)):
                    prob_grid = np.array(data.probability(1).energies)
                    prob      = np.zeros((len(prob_grid) - 1, N_dist))
                    for k in range(N_dist):
                        prob[:, k] = np.array(data.probability(k + 1).probabilities[:-1])
                else:
                    print_error(
    ...
    if fissionable:
        ...
        load_fission_multiplicity(
            prompt_block.multiplicity,
            fission_group.create_group("prompt_multiplicity")
        )
...
def process_pstar_only_file(symbol, A, awr, output_dir, pstar_dir, verbose=False):
    """
    Create a minimal HDF5 file for an isotope that has no ACE data but does
    have a PSTAR stopping power file. Returns the output filename, or None if
    no PSTAR file was found.
    """
    ...
    file.attrs["source_date"]    = "N/A"

    file.create_dataset("nuclide_name",        data=nuclide_name)
    file.create_dataset("excitation_level",    data=0)
    file.create_dataset("temperature",         data=T_KELVIN).attrs["unit"] = "K"
    file.create_dataset("atomic_number",       data=Z)
    file.create_dataset("mass_number",         data=A)
    file.create_dataset("atomic_weight_ratio", data=awr)
    file.create_dataset("radiation_length",    data=radiation_length).attrs["unit"] = "g/cm2"
...
def main():
    parser = argparse.ArgumentParser(
        description="MC/DC proton data generator"
    )
    ...
    if ace_dir is None:
        print_error("Environment variable $MCDC_ACELIB_PROTON is not set.")
    if pstar_dir is None:
    ...
    if pstar_dir is not None:
        ...
        for symbol, A, awr in PSTAR_ONLY_ISOTOPES:
            ...
            try:
                ...
                if out is None:
                    if verbose:
                        print(f"  [skip] No PSTAR data for {nuclide_name}")
                elif verbose:
                    print(f"  -> wrote {out}  [stopping power only]")
            except Exception as exc:
                errors.append((nuclide_name, str(exc)))
                if verbose:
                    import traceback
                    traceback.print_exc()
    ...
    n_total = len(target_files) + len(PSTAR_ONLY_ISOTOPES)
    print(f"\nDone. {n_total - len(errors)} succeeded, {len(errors)} failed.")
    if errors:
        print("\nFailed files:")
